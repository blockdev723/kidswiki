...
import { StyleSheet, Text, View, Image, ScrollView, WebView, TouchableOpacity, ImageBackground, AsyncStorage, FlatList, TextInput } from 'react-native';
...
export class SearchList extends Component {
  ...
  componentWillMount() {
    ...
    var itemId = navigation.getParam('interest');
    var displayTitle = navigation.getParam('displayTitle');
    var titleForApi = navigation.getParam('titleForApi');
    var selectedChild = navigation.getParam('selectedChild');

    console.log("itemId======>>>>>", JSON.stringify(itemId) + "\n" + titleForApi)
    this.setState({ spinner: true, itemId: itemId, titleForApi: titleForApi, displayTitle: displayTitle });
    AsyncStorage.getItem("token").then((value) => {
      AsyncStorage.getItem('childs').then((childs) => {
        console.log("user_token home", value, childs)
        var childsss = JSON.parse(childs);
        this.setState({ user_token: value, childs: childsss, selectedChild: selectedChild })
        this.getRows(itemId);
        this.getUserList();
      }).done();
    }).done();
  ...
  getRows = (itemId) => {
    ...
    fetch(Base_Url + 'loadMoreArticles', {
      method: "POST",
      headers: {
      ...
      .then((responseJson) => {
        ...
        this.setState({ spinner: false, articlesName: responseJson.list });
        console.log("finallllll", this.state.articlesName);
      }).catch((error) => {
        console.log(error);
  ...
  setShowDescriptionModal(item, index) {
    ...
    this.setState({ visibleModal: 'sliding', htmlForm: '', responseModal: '', spinner2: true, articlesToOpen: articlesToOpen, seachedItem: item, selectedIndex: index, transparentModal: true });
    //  console.log("==========>>>>", this.state.titleForApi)
    ...
    fetch(Base_Url + 'getArticleDetail', {
      ...
      body: JSON.stringify({
        ...
        childId: this.state.selectedChild.id,
        interestName: this.state.titleForApi
      })
    })
      ...
      .then((responseJson) => {
        if (responseJson.status == 0) {
          ...
          this.setState({ htmlForm: openFacts, responseModal: responseJson, htmlFormPrev: openFacts, spinner: false });
  ...
  addToFavorite(item, index) {
    ...
    this.state.articlesName[index].isFavourite = favoArrayData;
    console.log("Toppp=======>>>>>>", this.state.articlesName[index].isFavourite = favoArrayData)
  ...
  addToFavoriteeeeeee(item, index) {
        ...
        <View style={{ flex: 1 , alignItems: 'center', justifyContent:'center'  }}>
          <View style={{ width: '100%', flexDirection: 'row', backgroundColor: 'white', alignItems: 'center', position: 'absolute', top: -30, left: 20, right: 20, zIndex: 999 }}>
            ...
            <View style={{ width: '100%', flexDirection: 'column', marginLeft: 20 }}>
              <Text style={{ textTransform: 'uppercase', color: '#527CF3', fontSize: 25, fontFamily: 'BigJohn', marginLeft: 5 }}>Articles related to {this.state.displayTitle}</Text>
              ...
              <View style={{ flex: 1, backgroundColor: '#fff', marginTop: 10 }}>
                ...
                <View style={{ position: 'absolute', marginTop: 15, right: 10, flexDirection: 'row', flexDirection: 'row', padding: 10 }}>
                  <View style={{ position: 'absolute', right: 10, flexDirection: 'row' }}>
                    ...
                    <TouchableOpacity style={{ marginRight: 15 }} onPress={(nn) => this.addToFavoriteeeeeee(this.state.seachedItem, this.state.selectedIndex)}>
